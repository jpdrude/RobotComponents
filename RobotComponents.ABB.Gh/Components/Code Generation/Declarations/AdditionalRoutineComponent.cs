...
namespace RobotComponents.ABB.Gh.Components.CodeGeneration
{
    ...
    /// </summary>
    public class AdditionalRoutineComponent : GH_RobotComponent
    {
        #region fields
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //Generates Output
            Routine method = new Routine(actions, (RoutineType)type, name, (Scope)scope);

            //Generates Routine Call if PROC
            List<CodeLine> routineCall = new List<CodeLine>();
            if (method.Type == RoutineType.PROC)
            {
                routineCall.Add(new CodeLine($"{method.Name};", CodeType.Instruction));
            }
