...
{
    /// <summary>
    /// Represents the Timed Path Generator.
    /// </summary>
    /// <remarks>
    /// This class approximates the path a Robot will follow for a given set of Actions.
    /// Instead of a fixed subdivision count, it accepts a time interval in seconds.
    /// Each movement is subdivided based on how long it takes to execute (path length / TCP speed),
    /// so faster or shorter movements produce fewer steps and slower or longer ones produce more.
    /// </remarks>
    public class TimedPathGenerator
    {
        #region fields
        private readonly Robot _robot; // The robot info to construct the path for
        private readonly List<Plane> _planes; // The planes the path follows
        ...
        private bool _isFirstMovementMoveAbsJ; // Bool that indicates if the first movement is an absolute joint movement
        ...
        private double _timeInterval = 0.1; // Time interval in seconds between interpolation steps
        private double _programTime = 0; // Estimate of the total program time
        #endregion

        #region constructors
        /// <summary>
        /// Initializes an empty instance of the Timed Path Generator class.
        /// </summary>
        public TimedPathGenerator()
        {
        }

        /// <summary>
        /// Initializes a new instance of the Timed Path Generator class.
        /// </summary>
        /// <param name="robot"> The Robot to generate the path for. </param>
        public TimedPathGenerator(Robot robot)
        {
        ...
        public override string ToString()
        {
            if (!IsValid)
            {
                return "Invalid Timed Path Generator";
            }
            else
            {
                return "Timed Path Generator";
            }
        }
        ...
        /// The time interval in seconds between interpolation steps. Each movement is subdivided
        /// based on its duration (path length / TCP speed), so that one step approximates this
        /// time interval. Minimum 1 step per movement is always guaranteed.
        /// </param>
        public void Calculate(IList<Actions.IAction> actions, double timeInterval = 0.1)
        {
            _robot.ForwardKinematics.HideMesh = true;
            _timeInterval = Math.Max(0.001, timeInterval); // guard against zero / negative
            int counter = 0;
            int firstN = 1; // interpolation count of the first movement, used for RemoveRange
            ...
            for (int i = 0; i < ungrouped.Count; i++)
            {
                ...
                {
                    int planesBefore = _planes.Count;
                    ...
                    // Track how many steps the first movement produced for RemoveRange below
                    if (!firstMovementDone && counter > 0)
                    {
                        firstN = _planes.Count - planesBefore;
                        firstMovementDone = true;
                    }
                }
            }

            // Remove first dummy values (the initial home position plus the first movement steps,
            // keeping only the endpoint of the first movement as the output start position)
            if (counter > 0)
            {
                _movements.RemoveRange(0, firstN);
                _robotJointPositions.RemoveRange(0, firstN);
                _externalJointPositions.RemoveRange(0, firstN);
                _configurationDatas.RemoveRange(0, firstN);
                _planes.RemoveRange(0, firstN);
                _isInLimits.RemoveRange(0, firstN);
            }
        ...
        private void JointMovementFromJointTarget(Movement movement)
        {
            ...
            // Estimate path length via FK at the target joint position
            _robot.ForwardKinematics.Calculate(towardsRobotJointPosition, towardsExternalJointPosition);
            double pathLength = _planes.Last().Origin.DistanceTo(_robot.ForwardKinematics.TCPPlane.Origin);
            int n = ComputeInterpolations(pathLength, movement.SpeedData, movement.Time);

            // Interpolate
            InterpolateJointMovement(towardsRobotJointPosition, towardsExternalJointPosition, movement, n);
        }
        ...
        private void JointMovementFromRobotTarget(Movement movement)
        {
            ...
            // Estimate path length via FK on the IK result
            _robot.ForwardKinematics.Calculate(towardsRobotJointPosition, towardsExternalJointPosition);
            double pathLength = _planes.Last().Origin.DistanceTo(_robot.ForwardKinematics.TCPPlane.Origin);
            int n = ComputeInterpolations(pathLength, movement.SpeedData, movement.Time);
        ...
        private void LinearMovementFromRobotTarget(Movement movement)
        {
            ...
            // Linear curve (in work object coordinate space)
            Line line = new Line(plane1.Origin, plane2.Origin);

            // Compute number of interpolation steps based on movement duration
            int n = ComputeInterpolations(line.Length, movement.SpeedData, movement.Time);

            // External Joint Position change
            ExternalJointPosition externalJointPositionChange = CalculateExternalJointPositionChange(towardsExternalJointPosition, n);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // Add the calculated joint positions and plane to the class property
        ...
        /// </summary>
        /// <param name="movement"> The movement as a circular movement type. </param>
        private void CircularMovementFromRobotTarget(Movement movement)
        {
            ...
            // Circular curve (in work object coordinate space)
            ...
            // Compute number of interpolation steps based on movement duration
            int n = ComputeInterpolations(arc.Length, movement.SpeedData, movement.Time);
        ...
        /// <param name="n"> The number of interpolation steps for this movement. </param>
        private void InterpolateJointMovement(RobotJointPosition towardsRobotJointPosition, ExternalJointPosition towardsExternalJointPosition, Movement movement, int n)
        {
            // Calculate the joint position value change per interpolation
            RobotJointPosition robotJointPositionChange = CalculateRobotJointPositionChange(towardsRobotJointPosition, n);
            ExternalJointPosition externalJointPositionChange = CalculateExternalJointPositionChange(towardsExternalJointPosition, n);
        ...
        /// <summary>
        /// Computes the number of interpolation steps for a movement based on its estimated duration.
        /// </summary>
        /// <param name="pathLength"> The path length in millimetres. </param>
        /// <param name="speedData"> The speed data of the movement. </param>
        /// <param name="time">
        /// The fixed time override of the movement (from <see cref="Movement.Time"/>).
        /// When non-negative this value is used directly instead of <paramref name="pathLength"/> / V_TCP.
        /// </param>
        /// <returns> The number of interpolation steps (at least 1). </returns>
        private int ComputeInterpolations(double pathLength, SpeedData speedData, double time)
        {
            double movementTime;
            ...
            else if (speedData.V_TCP > 0)
            {
                movementTime = pathLength / speedData.V_TCP;
            }
            else
            {
                return 1;
            ...
            return Math.Max(1, (int)Math.Ceiling(movementTime / _timeInterval));
        }

        /// <summary>
        /// Calculates the change in robot joint position per interpolation step.
        /// </summary>
        ...
        private RobotJointPosition CalculateRobotJointPositionChange(RobotJointPosition towardsRobotJointPosition, int n)
        {
            ...
            robotJointPositionChange.Divide(n);

            return robotJointPositionChange;
        }

        /// <summary>
        /// Calculates the change in external joint position per interpolation step.
        /// </summary>
        ...
        private ExternalJointPosition CalculateExternalJointPositionChange(ExternalJointPosition towardsExternalJointPosition, int n)
        {
            ...
            externalJointPositionChange.Divide(n);
        ...
        /// <returns>
        /// A <see cref="ConfigurationData"/> object containing the calculated configuration data.
        /// </returns>
        ...
        /// <summary>
        /// Gets the path curve as list with curves.
        /// </summary>
        /// <remarks>
